...
(function () {
  ...
  function serverTower(parent, gx, gy, w, d, h, base, glow, units) {
    ...
    for (let i = 0; i < units; i++) {
      ...
      // front-left face (y = yM) and front-right face (x = xM)
      poly(parent, [R(xm, yM, z1), R(xM, yM, z1), R(xM, yM, z0), R(xm, yM, z0)], shade(base, -0.02));
      poly(parent, [R(xM, ym, z1), R(xM, yM, z1), R(xM, yM, z0), R(xM, ym, z0)], shade(base, -0.22));
      // thin dark seam between units on both faces
      el("line", { x1: R(xm, yM, z1)[0], y1: R(xm, yM, z1)[1], x2: R(xM, yM, z1)[0], y2: R(xM, yM, z1)[1],
        stroke: shade(base, -0.34), "stroke-width": 1 }, parent);
      // glowing light bar across the right face, near the top of the unit
      const b1 = z1 - uh * 0.16, b0 = z1 - uh * 0.34;
      poly(parent, [R(xM, ym, b1), R(xM, yM, b1), R(xM, yM, b0), R(xM, ym, b0)], glow.bar);
      // indicator LEDs on the left face, near the bottom of the unit
      const l1 = z0 + uh * 0.52, l0 = z0 + uh * 0.30;
      for (let k = 0; k < 3; k++) {
        const a0 = xm + w * (0.24 + 0.24 * k), a1 = a0 + w * 0.11;
        poly(parent, [R(a0, yM, l1), R(a1, yM, l1), R(a1, yM, l0), R(a0, yM, l0)], k === 1 ? glow.led : "#F2F8FF");
      }
    }
