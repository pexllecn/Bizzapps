...
(function (root) {
  ...
  var M = {
    ...
    /* The wash is drawn in the additive glow pass, not as surface, so it is
       given the colour the light actually is: a warm near-white. As opaque
       geometry this had to be a dark tone carried by its emissive term, and
       it still read as four solid stripes leaning on the board. */
    beamGlow: hex("#FFE2AE"),
    stone:    hex("#9AA0B2"),
    stoneLt:  hex("#B4BAC9"),
    glass:    hex("#66708A"),
  ...
  LM.eyhq = function (b, x, y, tone, id, decals) {
    ...
    /* ---- the rank that lights it ----
       ...
       one throws a soft cone of light up on to the face, drawn in the
       additive glow pass: crossed cards that feather to nothing at their
       edges, brightest at the lens and thinning as the cone opens. Nothing
       here is solid, so a beam brightens whatever is behind it and never
       covers it.

       The earlier version built each wash as opaque wedge geometry. It read
       as four hard-edged stripes with visible outlines, and from some
       bearings it cut across the mark. */
    var FX = SX + 0.32, FZ = aTop + 0.10;
    /* the cone lands on the board just short of the plane the decal sits on,
       so the mark is lit rather than crossed */
    var TZ = SZ0 + BH * 0.62, TXB = SX + (STH + 0.05) / 2 + 0.012;
    for (var si = 0; si < 4; si++) {
      var u  = si / 3 - 0.5;
      var fy = SY + u * BW * 0.74;

      /* the fixture: a small dark body and a lit lens */
      b.box(FX, fy, FZ, 0.09, 0.09, 0.11, M.hqCol, id, 0.85);
      b.emis(0.55);
      b.prism(FX, fy, FZ + 0.11, 0.055, 0.048, 0.075, 8, 0, M.beam, id, 1);
      b.emis(0);

      /* Each cone leans towards the centre of the mark, but only part of the
         way, so the four stay readable as four fixtures rather than merging
         into one slab of light across the bottom of the board. */
      var ty = SY + u * BW * 0.50;
      var p0 = [FX - 0.02, fy, FZ + 0.175];
      var p1 = [TXB, ty, TZ];

      /* the shaft itself, opening from the lens to the width of its pool */
      b.glowShaft(p0, p1, 0.030, BW * 0.15, M.beamGlow, 0.20, 0.09, 3);
      /* a tighter, brighter core inside it - a real beam is not one flat
         density, and this is what stops it reading as a painted band */
      b.glowShaft(p0, p1, 0.016, BW * 0.075, M.beamGlow, 0.18, 0.07, 2);

      /* halo at the lens, so the source is the brightest thing in the rank */
      b.glowSprite([FX - 0.02, fy, FZ + 0.16],
                   [0, 1, 0], [0, 0, 1], 0.30, 0.30, M.beamGlow, 0.45);
      /* the pool the beam throws on the face of the board, stood just off it
         so it grazes the panel and the mark instead of sitting behind them */
      b.glowSprite([TXB + 0.006, ty, SZ0 + BH * 0.42],
                   [0, 1, 0], [0, 0, 1], BW * 0.42, BH * 0.86,
                   M.beamGlow, 0.20);
    }
    /* a low wash of spill on the deck under the rank, which is what sells
       the fixtures as sitting on a surface rather than floating */
    b.glowSprite([FX + 0.06, SY, FZ + 0.012], [0, 1, 0], [1, 0, 0],
                 BW * 0.95, 0.52, M.beamGlow, 0.12);
